...
export const ScanTotpScreen = () => {
    return (
        <BaseAuthentication
            ...
            <div className="flex flex-row justify-between xl:my-6 mt-2 mb-4 h-16 w-full xl:gap-8 gap-4">
                {Array.from({length: 6}).map((_, index) => (
                    <input
                        ...
                        className="border-2 border-strokeGrey text-2xl font-semibold rounded-lg min-w-10 max-w-12 flex-grow aspect-[3/4] h-auto bg-transparent text-center text-onBackground focus:outline-none focus:border-secondary"
                    ></input>
                ))}
            </div>
            <button
            ...
            <button
                className="border-2 border-strokeGrey text-white flex-grow w-full max-h-14 rounded-lg text-[24px] mt-4">Skip
            </button>
        </BaseAuthentication>
    )
}
